...
function UnlockModal({
  ...
  const [step, setStep] = useState<"confirm" | "unlocking" | "done">("confirm");
      ...
      >
        ...
        {step === "unlocking" && (
          <div className="p-6 flex flex-col items-center py-12">
            ...
            <p className="font-bold text-lg" style={{ color: "#0F172A" }}>Unlocking...</p>
            <p className="text-sm mt-1" style={{ color: "#94A3B8" }}>Setting up your chat</p>
          </div>
        )}
...
export default function RoommateRooms() {
  ...
  const filtered = ROOMS_DATA.filter(r =>
    filter === "available" ? r.occupiedBeds < r.totalBeds : true
  );
